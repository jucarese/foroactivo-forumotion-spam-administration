(() => {
  ...
  const rootMarkup = `
    ...
    <section id="fa-purge-panel" role="dialog" aria-modal="true" aria-labelledby="fa-purge-title">
      <header id="fa-purge-head">
        <h2 id="fa-purge-title">Spam Administration · Forumotion</h2>
        <button id="fa-purge-close" type="button" title="Close">×</button>
      </header>
      <div id="fa-purge-body">
        <label for="fa-purge-user">Exact username</label>
        <input id="fa-purge-user" type="text" autocomplete="off">
        <p id="fa-purge-note">The tool will scan first without deleting anything.</p>
        <div id="fa-purge-actions">
          <button id="fa-purge-scan" type="button">Scan posts</button>
          <button id="fa-purge-cancel" type="button" disabled>Cancel process</button>
        </div>
        <div id="fa-purge-summary"></div>
        <div id="fa-purge-warning">
          This operation cannot be undone. Topics started by the user will be deleted
          in full, including replies posted by other members.
        </div>
        <div id="fa-purge-report-wrap" hidden>
          <div id="fa-purge-actions">
            <button id="fa-purge-report" type="button">Download required pre-deletion report</button>
          </div>
        </div>
        <div id="fa-purge-confirm-wrap" hidden>
          <label for="fa-purge-confirm">To confirm, type DELETE <span></span></label>
          <input id="fa-purge-confirm" type="text" autocomplete="off">
          <div id="fa-purge-actions">
            <button id="fa-purge-delete" type="button" disabled>Delete permanently</button>
          </div>
        </div>
        <progress id="fa-purge-progress" value="0" max="1"></progress>
        <div id="fa-purge-log" aria-live="polite">Tool ready. Nothing has been deleted.</div>
      </div>
    </section>`;
  ...
  function confirmationIsValid() {
    ...
    return normalize(ui.confirm.value) === normalize(`DELETE ${state.username}`);
  }

  function safeFilePart(value) {
    return value.replace(/[^a-z0-9_-]+/gi, "_").replace(/^_+|_+$/g, "").slice(0, 60) || "user";
  }
  ...
  async function request(url, options = {}) {
    if (state.cancelled) throw new Error("Process cancelled.");
    if (!sameOrigin(url)) throw new Error("An external address was blocked.");
  ...
  function collectExternalUrls(container) {
    ...
    for (let value of matches) {
      ...
      } catch (_) { /* URL-like text, but invalid */ }
    }
    return [...urls];
  }
  ...
  function buildReport(finalReport) {
    ...
    const lines = [
      "SPAM LOG · FORUMOTION",
      "================================",
      `Type: ${finalReport ? "final result" : "pre-deletion report"}`,
      `Local date: ${now.toLocaleString()}`,
      `ISO date: ${now.toISOString()}`,
      `Forum: ${location.origin}`,
      `PRIMARY IDENTIFIER: ${canonicalProfileUrl(state.userId) || state.profileUrl || "not found"}`,
      `USERNAME: ${state.username}`,
      `NUMERIC ID: ${state.userId || "not found"}`,
      `Posts found: ${state.posts.size}`,
      `Topics started: ${state.topicStarters.length}`,
      `Replies in other members' topics: ${state.replies.length}`,
      `Unique external URLs: ${state.externalUrls.size}`,
      "",
      "VISIBLE PROFILE DATA",
      "--------------------------",
      ...(state.profileDetails.length ? state.profileDetails : ["No visible profile data could be extracted."]),
      "",
      "EXTERNAL URLS POSTED",
      "-----------------------",
      ...([...state.externalUrls].length ? [...state.externalUrls].sort() : ["No external URLs were found."]),
      "",
      "POSTS FOUND",
      "-------------------------"
    ];
    for (const item of state.evidence) {
      lines.push(
        ...
        `External URLs: ${item.externalUrls.length ? item.externalUrls.join(" | ") : "none"}`,
        ""
      );
    }
    if (finalReport) {
      lines.push("DELETION RESULT", "---------------------");
      if (!state.deletionResults.length) lines.push("No deletion was performed.");
      for (const result of state.deletionResults) {
        lines.push(
          `${result.ok ? "DELETED" : "ERROR"} · p${result.postId} · t${result.topicId}`,
          result.detail,
          ""
        );
      }
      if (state.cancelled) lines.push("Process cancelled by the user.");
    }
    lines.push(
      "NOTE",
      "----",
      "This file was generated locally. It contains no cookies, passwords, or session keys.",
      "The URLs can be copied into the official Forumotion tool to report the spam."
    );
    return lines.join("\r\n");
  }
  ...
  function renderSummary() {
    ...
    addLine("User", state.username);
    addLine("Identifier", canonicalProfileUrl(state.userId) || state.profileUrl || "not found");
    addLine("Topics started by the user", state.topicStarters.length);
    addLine("Replies by the user in other members' topics", state.replies.length);
    addLine("Posts found", state.posts.size);
    addLine("External URLs found", state.externalUrls.size);

    if (state.failures.length) {
      const warning = document.createElement("div");
      warning.style.color = "#a12222";
      warning.textContent = `Uncertain checks: ${state.failures.length}`;
  ...
  async function walkSearchPages(firstPage, label, collector) {
    ...
    for (let count = 0; count < 1000 && page; count++) {
      ...
      visited.add(key);
      collector(page.doc);
      log(`${label} ${count + 1}: ${state.posts.size} unique posts found.`);
  ...
  async function resolveSearchUrl(username) {
    ...
    for (const candidate of candidates) {
      ...
      if (detectLogin(result.doc)) throw new Error("You are not signed in to the forum.");
      ...
      diagnostics.push(`${candidate}: ${messages.length} posts, ${topics} topic links`);
      if (messages.length || extractUserId(result.doc, username)) return result;
    }
    throw new Error(
      "Forumotion did not return recognizable posts. Diagnostic: " +
      diagnostics.join(" | ")
    );
  }
  ...
  async function scan() {
    ...
    if (!state.username) throw new Error("Enter the exact username.");
    ...
    ui.progress.value = 0;
    setBusy(true);
    log(`Searching for posts by “${state.username}”…`);
    ...
    if (state.profileUrl) {
      try {
        ...
        state.failures.push(`Profile: ${error.message}`);
      }
    }
    await walkSearchPages(page, "Posts page", doc => {
      for (const post of collectPostLinks(doc)) state.posts.set(post.postId, post);
    });

    log("Also searching for every topic started by the user…");
    const startedTopics = new Map();
    const topicSearch = await resolveStartedTopicsUrl(state.username);
    if (topicSearch) {
      await walkSearchPages(topicSearch, "Topics page", doc => {
    ...
    for (const topic of startedTopics.values()) {
      try {
        ...
        state.failures.push(`Started topic ${topic.topicId}: ${error.message}`);
      }
    }

    if (!state.posts.size) throw new Error("No posts were found for that user.");

    const posts = [...state.posts.values()];
    ui.progress.max = posts.length;
    ui.progress.value = 0;
    log("Checking posts and collecting evidence before deletion…");
    ...
    for (const post of posts) {
      ...
      if (!topicFirstPosts.has(post.topicId)) {
        try {
          ...
          state.failures.push(`Topic ${post.topicId}: ${error.message}`);
        }
      }
      ...
      try {
        ...
        state.evidence.push({
          ...
          externalUrls,
          kind: topicFirstPosts.get(post.topicId) === post.postId ? "FULL TOPIC" : "REPLY"
        });
      } catch (error) {
        state.failures.push(`Evidence p${post.postId}: ${error.message}`);
      }
      ui.progress.value++;
    ...
    ui.report.textContent = "Download required pre-deletion report";
    ui.confirmLabel.textContent = state.username;
    ui.confirm.value = "";
    ui.confirmWrap.hidden = false;
    setBusy(false);
    log("Scan complete. Download the pre-deletion report before enabling deletion.");
  }
  ...
  async function deletePost(post) {
    ...
    if (detectLogin(topicPage.doc)) throw new Error("The session has expired.");
    const deleteUrl = findDeleteUrl(topicPage.doc, post.postId);
    if (!deleteUrl) throw new Error("The native delete action is not available; check the permissions.");

    const confirmation = await getDocument(deleteUrl);
    if (detectLogin(confirmation.doc)) throw new Error("The session has expired.");
    const form = confirmation.doc.querySelector(
      'form[action*="mode=delete"], form[action*="/post"], form[action*="delete"]'
    );
    if (!form) throw new Error("The Forumotion confirmation form was not found.");
    ...
    if (/not authorised|no est[aá]s autorizado|no tienes permiso|hacking attempt/i.test(text)) {
      throw new Error("Forumotion rejected the deletion.");
    }
    ...
    if (repeatedConfirmation) {
      throw new Error(
        `Forumotion returned the confirmation page again without deleting. Destination: ${response.url}`
      );
    }
    ...
    if (!verificationResponse.ok) {
      throw new Error(`The deletion could not be verified: HTTP ${verificationResponse.status}`);
    }
    ...
    if (detectLogin(verification.doc)) throw new Error("The session expired during verification.");
    ...
    if (targetStillExists) {
      const pageTitle = (resultDoc.title || "untitled").replace(/\s+/g, " ").trim();
      throw new Error(
        `Forumotion responded, but p${post.postId} is still visible. ` +
        `Response: ${response.url} · ${pageTitle}`
      );
    }
  }
  ...
  async function removeAll() {
    ...
    ui.progress.value = 0;
    log(`Starting deletion of ${topics.length} complete topics and ${replies.length} replies.`);
    ...
    for (let index = 0; index < queue.length; index++) {
      ...
      try {
        ...
        state.deletionResults.push({
          ...
          detail: "Forumotion confirmed that the content is gone."
        });
        log(`${index + 1}/${queue.length}: deleted p${item.postId}.`);
      } catch (error) {
        ...
        log(`${index + 1}/${queue.length}: ERROR on p${item.postId}: ${error.message}`);
        if (/session has expired/i.test(error.message)) break;
      }
    ...
    ui.remove.disabled = true;
    if (state.cancelled) {
      log("Process stopped by the user.");
    } else if (state.failures.length) {
      log(`Process finished with ${state.failures.length} items not deleted. Review the log.`);
    } else {
      log("Process finished with no detected errors.");
    }
    downloadText(
      `forumotion-spam-${safeFilePart(state.username)}-final-log.txt`,
      buildReport(true)
    );
    ui.report.textContent = "Download the final log again";
  }
  ...
  ui.cancel.addEventListener("click", () => {
    ...
    log("Cancellation requested; the process will stop after the current operation.");
  });
  ...
  ui.report.addEventListener("click", () => {
    ...
    downloadText(
      `forumotion-spam-${safeFilePart(state.username)}-${finalReport ? "final-log" : "pre-deletion-report"}.txt`,
      buildReport(finalReport)
    );
    if (!finalReport) {
      state.reportDownloaded = true;
      ui.remove.disabled = state.running || !confirmationIsValid();
      log("Pre-deletion report downloaded. You can now confirm deletion.");
    }
  });
